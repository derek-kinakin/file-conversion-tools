...
if __name__ == "__main__":
    ...
    # TODO ui.json to be handled by geoh5py, but just raw json for now...
    with open(file, encoding="utf-8") as file:
        ui_json = json.load(file)

    files_choice = ui_json["file_choice"]["value"].split(";")

    wp = Workspace(ui_json["geoh5"])
    ...
    if files_choice[0].lower().endswith(".dxf"):
        ext = "dxf"
    elif files_choice[0].lower().endswith(".ply"):
        ext = "ply"
    else:
        raise ValueError("No valid extension selected.")

    if ext == "dxf":
        with tqdm(total=len(files_choice), ncols=80, desc="DXF>GEOH5") as pbar:
            for dxf in files_choice:
                dxf_to_geoh5(wp, dxf)
                pbar.update()
    else:
        with tqdm(total=len(files_choice), ncols=80, desc="PLY>GEOH5") as pbar:
            for ply in files_choice:
                ply_to_geoh5(wp, ply)
                pbar.update()